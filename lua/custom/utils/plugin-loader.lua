...
local function safe_require(module_path)
  ...
  if not ok then
    vim.notify(
      string.format('Failed to load plugin module: %s\nError: %s', module_path, result),
...
local function validate_plugin_config(config, module_path)
  if type(config) ~= 'table' then
    vim.notify(
      string.format('Invalid plugin config in %s: expected table, got %s', module_path, type(config)),
      vim.log.levels.WARN
    )
    return false
  end
